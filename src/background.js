...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'addReaction') {
    ...
    handleTokenSaved(request.token, sendResponse)
    return true
  }
})
...
async function handleAddReaction(request, sendResponse) {
  try {
    console.log('Handling addReaction request:', request)
...
// Handle token saved notification
async function handleTokenSaved(token, sendResponse) {
  try {
    await storeToken(token)
    console.log('Token saved in background script')
    sendResponse({ success: true, message: 'Token saved successfully' })
  } catch (error) {
    console.error('Error saving token in background script:', error)
    sendResponse({ success: false, error: error.message })
  }
}
...
async function storeToken(token) {
  return new Promise((resolve) => {
    console.log('Storing token in background script:', token ? 'Token provided' : 'No token')
    chrome.storage.local.set({ slackToken: token }, () => {
      console.log('Token stored successfully in chrome.storage.local')
      resolve()
    })
  })
}
...
async function getStoredToken() {
  return new Promise((resolve) => {
    console.log('Getting token from chrome.storage.local...')
...
chrome.runtime.onStartup.addListener(async () => {
  ...
  const token = await getStoredToken()
  console.log('Startup: Token status:', token ? 'Token exists' : 'No token found')
})

chrome.runtime.onInstalled.addListener(async () => {
  console.log('Slack Reaction Tool: Extension installed')
  const token = await getStoredToken()
  console.log('Install: Token status:', token ? 'Token exists' : 'No token found')
}) 